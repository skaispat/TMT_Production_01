...
export function FullKittingModal({ isOpen, onClose, recordData, onProcessed, showToast }) {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black bg-opacity-50 flex items-start sm:items-center justify-center p-1 sm:p-4 overflow-y-auto">
      <form onSubmit={handleSubmit}>
        <div
          ref={setModalRef}
          className="bg-white rounded-lg shadow-xl max-w-5xl w-full max-h-[95vh] overflow-y-auto mx-1 sm:mx-4 my-2 sm:my-4"
        >
          {/* Modal Header */}
          <div className="flex justify-between items-center p-2 sm:p-4 border-b border-slate-200">
          ...
          <div className="p-2 sm:p-4">
            ...
            <div className="overflow-x-auto -mx-4 sm:mx-0 pb-2">
              <div className="inline-block min-w-full align-middle">
                <table className="min-w-full border-collapse table-fixed sm:table-auto">
                  <thead>
                    <tr className="bg-slate-100 ">
                      <th className="border border-slate-300 p-1 sm:p-2 text-left w-10 sm:w-16 text-xs sm:text-sm">
                        SI no
                      </th>
                      <th className="border border-slate-300 p-1 sm:p-2 text-left text-xs sm:text-sm">Particulars</th>
                      <th className="border border-slate-300 p-1 sm:p-2 text-left text-xs sm:text-sm">Yield</th>
                      <th className="border border-slate-300 p-1 sm:p-2 text-left text-xs sm:text-sm">Fem</th>
                      <th className="border border-slate-300 p-1 sm:p-2 text-left text-xs sm:text-sm">Price</th>
                      <th className="border border-slate-300 p-1 sm:p-2 text-left bg-yellow-100 text-xs sm:text-sm">
                        %
                      </th>
                      <th className="border border-slate-300 p-1 sm:p-2 text-left text-xs sm:text-sm">Yield</th>
                      <th className="border border-slate-300 p-1 sm:p-2 text-left text-xs sm:text-sm">Fem</th>
                  ...
                  <tbody>
                    {rows.map((row) => (
                      <tr key={row.id} className={row.id % 2 === 0 ? "" : "bg-yellow-50"}>
                        <td className="border border-slate-300 p-1 sm:p-2 text-xs sm:text-sm">{row.id}</td>
                        <td className="border border-slate-300 p-1 sm:p-2">
                          <select
                            value={row.particulars}
                            onChange={(e) => updateRowData(row.id, "particulars", e.target.value)}
                            className="w-full px-1 py-1 border border-slate-300 text-xs sm:text-sm rounded-md shadow-sm focus:outline-none focus:ring-1 focus:ring-slate-500"
                          >
            ...
            {/* Modal Footer */}
            <div className="flex flex-col sm:flex-row justify-end gap-2 mt-2 sm:mt-4 pt-2 sm:pt-4 border-t border-slate-200 p-2 sm:p-4">
              <button
                type="button"
                onClick={onClose}
