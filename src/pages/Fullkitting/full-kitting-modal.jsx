        ...
        data.table.rows.slice(1).forEach((row) => {
          if (row.c && row.c[0] && row.c[0].v) {
            ...
            if (existingNumber.startsWith("CN-")) {
              const num = parseInt(existingNumber.substring(3))
  ...
  const handleSubmit = async (e) => {
    ...
    //   recordData[14] = totalPrice.toFixed(2) // O - Total Price
      ...
      materialsData.forEach((material, index) => {
        if (index < 14) { // Limit to 15 materials
          const columnIndex = 14 + index // Starting from O (14)
          if (columnIndex <= 28) { // Up to AC (28)
      ...
      if (result.success) {
        showToast && showToast("Success", "Full kitting data has been submitted successfully with composition number: " + compositionNumber)
        ...
        >
          ...
          <div className="flex justify-end gap-2 mt-4 pt-4 border-t border-slate-200 dark:border-slate-700">
            ...
            <button
              ...
              className="px-4 py-2 bg-slate-700 text-white rounded-md hover:bg-slate-800 focus:outline-none focus:ring-2 focus:ring-slate-500 disabled:opacity-50 disabled:cursor-not-allowed"
            >
